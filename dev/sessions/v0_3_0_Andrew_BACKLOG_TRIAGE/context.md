...
- **🚨 INCIDENT + ROLLBACK 2026-06-02 ~10:45pm — NO DATA LOST, collab REVERTED.** Andy opened agendas on the collab build and topic bodies displayed EMPTY (Pre-Brief, a non-collab RichBodyEditor field, survived → the tell). **Root cause: a DISPLAY/seeding bug, NOT data loss** — during the broken (pre-dedupe) period, `collabSeeded` got set true while the Yjs fragment stayed empty (seed silently failed), so later collab loads showed empty and the reseed was blocked. **Firestore `topic.bodyHtml` was INTACT the whole time** (the mirror never wrote empty — guarded unmount-flush + local-origin-only mirror). PROOF: reverted prod alias to the **pre-collab build `7zz0mzxc9`** (sticky-titles, RichBodyEditor reads Firestore bodyHtml directly) → hard-refresh → ALL content restored.
  - **CURRENT SAFE STATE: prod alias → `7zz0mzxc9` (pre-collab, stable). Collab is OFF.** Do NOT re-alias to a collab build until the seeding bug is fixed. Team must hard-refresh to get the stable build.
  - **BEFORE RE-ENABLING COLLAB — must fix:** the seed guard. `collabSeeded` must be set ONLY after the fragment is actually populated (verify `frag.length>0` post-setContent), never on a failed/empty seed; and on load, if `collabSeeded` is true but the Yjs fragment is empty AND Firestore bodyHtml is non-empty, RE-SEED (don't trust the flag blindly). Consider dropping the Firestore-flag approach for a content-presence check. The dual-yjs dedupe fix (`000fefe`) is correct and stays; the seed-guard logic is the remaining bug.
- **✅ ANDY-CONFIRMED real two-user test (2026-06-02) [on the now-reverted collab build]:** adeemer@ + seo@ on the same "Review/GMB Discussion" agenda — topic edits sync live both ways, presence avatars both directions, **live cursors with name labels** rendering. The earlier real-test failure was the OLD cached bundle on one side; **hard-refresh BOTH browsers after a deploy** to pick up the fix. Clean build `gnns4inpm` is prod (alias set). Diagnostic badge removed. FEATURE COMPLETE.
