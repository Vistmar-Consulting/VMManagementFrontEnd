...
- **Two-client prod verification (driven via 2 agent-browser tabs, Profile 10, GV – Biweekly):** page renders; all topic bodies SEED from Firestore; presence SYMMETRIC (both clients see each other, 3 avatars incl. nav); **live edit propagation CONFIRMED** (typed SYNCZZ in tab A → appeared in tab B ~2s); deletion/restore propagated both ways; reload PERSISTS clean with NO double-seed (exactly 1 occurrence); websocket stable (initial 1006 was a one-time reconnect, 0 recurrences). **Test data fully reverted** (Milpitas body restored to exact original "Chinese ad copy approved").
