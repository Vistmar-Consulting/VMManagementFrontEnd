...
## AI ENGINE COMPLETE (3a → 3a.2 → 3b → 5a) on the data foundation (F1 categories+SOPs, F2 tags, F3 200 real items).
- **NEXT / deferred:** Slice 5b (status-moves + notes on EXISTING tasks, needs selective per-change approval); editable task-suggestion prompt in Settings; full SOPs back in agenda gen (currently descriptions-only for speed — documented on AI Integration card); the two security deferrals (client-side prompt, FE-only admin gate); the portSeed.js stale numeric-tag-id latent trap (dormant; F3 wipes/replaces tags with name-ids).
