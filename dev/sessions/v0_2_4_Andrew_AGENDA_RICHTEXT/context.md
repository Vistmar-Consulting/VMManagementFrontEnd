...
### Slice 5a — AI-suggested Project Board tasks ✅ DONE + PROD-VERIFIED (2026-05-31)
Spec: `docs/superpowers/specs/2026-05-31-ai-suggest-tasks-5a-design.md`. Commits `c2d91cc` (feat) + `186ac59` (review fixes). Separate admin **"Suggest tasks"** button on the agenda (next to AI Gen).
- **`api/ai/suggest-tasks.js`** (new): reads meeting window + **full SOPs** + existing board titles (dedup) → structured `{tasks:[{title,category,tags,note}]}`. effort medium, maxDuration 300.
- **`src/lib/aiTasks.js`**: `suggestTasks` (POST) + `applyTaskSuggestions` (writes statusId-8 items in a **runTransaction** — atomic org itemNumber counter, resolves categoryId/tagIds, creates coined tags retry-safe, fractional order seeds fresh→sorts to top for triage). Exports `tagSlug` from aiAgenda.
- **`SuggestTasksDialog.jsx`**: extra-context → suggest → checkbox review (category/tag chips, new-tag flag, source note) → add selected.
- **Verified on prod (Unio):** Suggest tasks → proposed 1 genuinely net-new task ("Chase Clayton approval on Marina patient-comms email" · account-management · corporate-leadership/blocked, note citing source+owner) → Add → written as statusId-8 item with correct category/tags. Test item deleted after. Board renders statusId-8 in the **Active group** (board partitions only DONE/ARCHIVE — verified, no per-status column gating).

### Slice 5b — status-moves + notes on existing tasks ✅ DONE + PROD-VERIFIED (2026-05-31)
Commit `11fd752`. Extends Suggest tasks: function output gains `moves[]` ({itemId,title,toStatus,reason}) + `notes[]` ({itemId,title,note}); existing tasks sent WITH itemId so the model references them. `applyTaskChanges(orgSlug,{creates,moves,notes},uid)`: creates→statusId-8 (tx); moves→updateDoc statusId (+completedAt on Done); notes→append to item.description. Dialog: 3 selectable review sections (New tasks / Status updates / Notes) — nothing mutates unless checked. **Prod-verified on Unio:** proposed Dr-Lee-claim move Pending→Done (accurate, cited 5/26) + 2 notes on existing tasks; applied → verified statusId=5 + completedAt + note appended; **reset Unio board via re-running seedClientTasks** (wiped 71, recreated 71 — clean undo).

### FE-only confirmed + SOP toggle (2026-05-31)
Andy: the USER does everything in the FE, no Claude Code. Confirmed — AI Gen, Suggest tasks, prompt editor (Settings→AI Integration), Client SOPs editor (Settings→Client SOPs), all review/apply are in-app. Only DEV engine-internals (window logic, guardrails) need Claude Code. **Closed the one gap:** "Include full Client SOPs" checkbox in the AI Gen modal (default off=fast descriptions ~1min; on=full SOPs ~2min). Explainer card updated (no more "tell Claude Code" for SOPs).

### Window anchor + AI-Gen record ✅ DONE + PROD-VERIFIED + rule deployed (2026-05-31)
Re-auth note: `firebase` not on Andy's PATH — use `npx firebase login --reauth` (or `./node_modules/.bin/firebase`). aiGenLog rule (read/create active + **delete admin**) DEPLOYED. Modal shows "Last AI-generated <date> (<relative>) — reconciles since then" / "Not yet AI-generated…" (commit `bee3e3e`). **Bug caught+fixed in verification** (`ed249e6`): the fallback refactor left a stray `lastOccurrence` ref in summary.usedFallbackWindow → "lastOccurrence is not defined" at gen time (build didn't catch undefined var) → now `!meetingAnchor && !orgAnchor`. **Prod-verified:** gen works, modal shows the date after apply, lastAgendaGenAt set, aiGenLog entry persists, anchor advances. Unio fully restored after (topics + anchor cleared + test log deleted).
Commits `3f7998a` + `aceeadb`. Fixed the recurring-pattern gap: the window was anchored to the meeting's last Fireflies occurrence → gen-right-after-a-meeting missed the inter-cycle work. **New anchor chain** (in `assembleGenInputs`, `anchorField` param): (1) `lastAgendaGenAt`/`lastSuggestTasksAt` ("since I last reconciled") → (2) this meeting's previous occurrence (firefliesTitles) → (3) **the ORG's previous meeting of any kind** (new/imported/unmapped meetings — "since this client last met us") → (4) flat 21d cold-start. (2)+(3) reach back to the prior occurrence if the latest is <2 days old (gen right after a meeting → full last cycle, not empty). Andy-confirmed: new meetings anchor to "since the org last met."
- `applyProposal` sets `lastAgendaGenAt`; `applyTaskChanges(...,{agendaId})` sets `lastSuggestTasksAt`. Both **best-effort log** to `agendas/{id}/aiGenLog` (outside the atomic write — tolerates the rule not yet deployed; warns).
- UI: "gen {N} ago" near AI Gen button (`agenda.lastAgendaGenAt`); version-history dialog has an **"AI Gen activity"** list (reads aiGenLog). "Include full Client SOPs" toggle in AI Gen modal (5b).
- **PENDING:** `firestore.rules` adds `aiGenLog` (read/create active) — **NOT deployed** (firebase CLI creds lapsed again; need Andy `firebase login --reauth`, then `./node_modules/.bin/firebase deploy --only firestore:rules --project management-db9eb`). Until deployed, aiGenLog writes warn-skip; window anchor + last-gen display work regardless. Then verify: re-gen shows "since [gen time]" (summary.anchoredToGen) + activity log persists.

### Retired-meeting cleanup ✅ DONE + PROD-VERIFIED (2026-05-31)
Commit `d05bd80`. Retired meetings (departed Tate's leftover Google events that can't be deleted) hidden via an **`archived: true`** flag on their calendar_series; `Calendar.jsx` filters archived series out everywhere (the reconcile preserves the flag — its drift-update touches only specific fields). **Reusable mechanism for any future retired meeting: set calendar_series.archived=true.**
- Retired GV "GV-Vistamar Bi-Weekly Mtg" (series `njmli7cdgr98l7734eu9ju5lmp` + `_R20260520T193000`) → replaced by "GV – Biweekly" (`ea74a0v5at8tqo46ka8mtth368`). Retired BMD "BMD Marketing" (`boc4jg4q1l4bk3cadeoklcg02v` + `_R20260521T183000`) → "BMD - Biweekly" (`6295h9p4mu514u7fkqm0tgmr68` + `_R...`). All 4 retired series archived.
- Retired agendas had **empty firefliesTitles** (nothing literally mapped). Pointed the replacements' firefliesTitles at BOTH new + old titles so the retired meetings' historical Fireflies transcripts still feed AI Gen: GV→`["GV – Biweekly","GV-Vistamar Bi-Weekly Mtg"]`, BMD→`["BMD - Biweekly","BMD Marketing"]` (BMD base + _R agendas).
- Prod-verified: GV filter shows only "GV – Biweekly"; BMD only "BMD - Biweekly".

## AI ENGINE COMPLETE — 3a → 3a.2 → 3b → 5a → 5b, on the data foundation (F1 categories+SOPs, F2 tags, F3 200 real items). All FE-self-service, all human-reviewed, all reversible.
- **Deferred (live tracker):** editable task-suggestion prompt in Settings (5a/b prompt hardcoded in api/ai/suggest-tasks.js); the two security deferrals (client-side prompt, FE-only admin gate — internal-trusted, fine); portSeed.js stale numeric-tag-id latent trap (dormant; F3 reseeds tags with name-ids); the earlier rich-text Phase 1 deferred items (migration cleanup of old talkingPoints/notes/openFloor subcollections; duplicate base/_R agenda-doc cleanup; finish Fireflies ambiguous-title mappings).
