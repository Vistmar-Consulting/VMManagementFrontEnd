...
- **NEXT — 3b:** AI Gen assigns categories (the 10) + coins new tags during agenda gen, using category descriptions + SOPs (F1c folds in here). Then 5 (AI-Gen task create/promote, statusId 8). Security deferrals (client-side prompt, FE-only admin gate) still stand. Prompt trim done; SOPs in Settings.
