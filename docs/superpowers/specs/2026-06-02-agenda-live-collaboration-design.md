...
- `import { applyCors } from "./meetings/_lib/cors.js"` and `import { requireAuth } from "./meetings/_lib/auth.js"` (same helpers `api/ai/*` use; path is relative to `api/liveblocks-auth.js`). `applyCors` first; then `await requireAuth(req,res)` (Firebase ID token via `X-User-Token` + `@vistamarconsulting.com` + verified-email gate). `req.authUser` = `{uid,email,displayName}`.
